...
function renderTask(task2) {
    ...
    if (task2.priorityRating == "Low") taskPriority = lowPriority;
    else if (task2.priorityRating == "Medium") taskPriority = lowPriority;
  ...
  let labelColor = colors[Math.floor(Math.random() * colors.length)];*/ item.classList.add('kanban__column-task');
    item.innerHTML = "<div><span style='float:right'>" + task2.taskCategory + "</span>" + "<h4>" + task2.taskTitle + "</h4></div>" + "<div><p>" + task2.taskDescription + "</p></div>" + "<div><h4 style='float:right;'>" + task2.estimatedTime + " hrs</h4>" + taskPriority + "</div>";
